...
import { Printer, User, Bookmark, CalendarDays, Layers } from 'lucide-react';
...
const BulkStudentPlans: React.FC = () => {
  ...
  // Fix: Explicitly type groupedStudents to Record<string, Student[]> to ensure Object methods like keys/entries infer correct types
  ...
  return (
    <div className="bg-slate-200 min-h-screen font-['Tajawal'] pb-20 no-print-bg">
      ...
      <div className="flex flex-col items-center gap-10">
        {/* Fix: Explicitly cast Object.entries to [string, Student[]][] to resolve unknown type issues for 'students' variable */}
        {(Object.entries(groupedStudents) as [string, Student[]][]).map(([classTitle, students]) => (
          <div key={classTitle} className="w-full flex flex-col items-center gap-16">
            {/* عنوان الفصل (يظهر في العرض فقط) */}
            <div className="no-print w-full max-w-[210mm] bg-white p-6 rounded-[2rem] border-2 border-indigo-100 flex items-center justify-between shadow-sm">
               <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-indigo-50 text-indigo-600 rounded-xl flex items-center justify-center"><Layers size={24} /></div>
                  <h2 className="text-2xl font-black text-slate-800">{classTitle}</h2>
               </div>
               <span className="bg-slate-100 px-4 py-2 rounded-xl text-slate-500 font-black text-sm">عدد الطلاب: {students.length}</span>
            </div>
              ...
              return (
                <div key={student.id} className="student-page-container print:m-0 print:shadow-none break-after-page" style={{ pageBreakAfter: 'always' }}>
                  <div className="a4-page bg-white shadow-2xl p-[8mm] relative flex flex-col overflow-hidden border border-slate-100" style={{ width: '210mm', height: '297mm', boxSizing: 'border-box' }}>
                    ...
                    {/* العلامة المائية - Watermark مرئية بوضوح أكبر (8%) */}
                    {school.logoUrl && (
                    ...
                    {/* الترويسة - إزالة اللون الأزرق ودمج التاريخ بجوار الأسبوع */}
                    ...
                    {/* الجدول الدراسي - ارتفاع الحصص لضمان ظهور 7 حصص كاملة */}
                    ...
                    {/* التذييل محذوف لتوفير المساحة العمودية للجدول */}
                  </div>
                </div>
              );
            })}
