        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <div className="text-right">
              ...
              <p className="text-[10px] text-indigo-600 font-black uppercase">بوابة الخطط الأسبوعية</p>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto p-4 flex flex-col items-center">
        {!selectedClass ? (
          ...
          <div className="w-full flex justify-center py-4">
            <div className="origin-top scale-[0.45] sm:scale-[0.7] md:scale-100 mb-[-500px] sm:mb-[-150px] md:mb-0">
               <div className="a4-page bg-white shadow-2xl border p-[8mm] relative flex flex-col overflow-hidden" style={{ width: '210mm', height: '297mm', boxSizing: 'border-box' }}>
                ...
                <div className="relative z-10 grid grid-cols-3 gap-2 mb-3 border-b-2 border-slate-900 pb-2 items-center">
                  ...
                  <div className="flex flex-col items-center justify-center">
                    {school.logoUrl ? <img src={school.logoUrl} className="w-16 h-16 object-contain" alt="شعار" /> : <div className="w-14 h-14 border-2 border-dashed rounded-lg flex items-center justify-center text-[7pt] text-slate-300 font-black">شعار المدرسة</div>}
                    <div className="mt-1 bg-slate-900 text-white px-3 py-0.5 rounded-full">
                      <span className="text-[7pt] font-black">الخطة الدراسية الأسبوعية</span>
                    </div>
                  </div>

                  <div className="text-left space-y-0.5 font-bold text-[8.5pt]">
                    <p>الأسبوع: <span className="font-black text-indigo-700">{activeWeek?.name || "---"}</span></p>
                    <p>الصف: <span className="font-black">{selectedClass}</span></p>
                    <p className="text-[7pt] text-slate-400">التاريخ: {activeWeek ? `${formatToHijri(activeWeek.startDate)}` : '--'}</p>
                  </div>
                </div>

                {/* الجدول الدراسي - 7 حصص يومياً */}
                          ...
                          {PERIODS.map((period, pIdx) => {
                            ...
                            return (
                              <tr key={`${day.id}-${period}`} className={`h-[18.2px] border-b ${pIdx === 6 ? 'border-b-[1.5px] border-slate-900' : 'border-slate-200'}`}>
                                ...
                                <td className="border-l-[1px] border-slate-300 text-[9pt] font-black bg-slate-50/30">{period}</td>
                                <td className="border-l-[1px] border-slate-300 text-[9pt] font-black truncate px-1 text-indigo-900">{subject}</td>
                                <td className="border-l-[1px] border-slate-300 text-[8.5pt] leading-tight px-1.5 truncate font-bold text-slate-700 italic">{plan.lesson || '-'}</td>
                                <td className="border-l-[1px] border-slate-300 text-[8.5pt] leading-tight px-1.5 truncate font-bold text-slate-800">{plan.homework || '-'}</td>
                                <td className="text-[7.5pt] px-1 text-slate-400 font-black truncate italic">{plan.enrichment || '-'}</td>
                              </tr>
                            );
                          })}
                        </React.Fragment>
                ...
                {/* التذييل */}
                <div className="relative z-10 grid grid-cols-2 gap-4 h-[35mm]">
                   <div className="border-[1.5px] border-slate-900 p-3 bg-white rounded-lg">
                      <h3 className="text-[9.5pt] font-black mb-2 border-b border-slate-900 pb-1 text-center bg-slate-50">توجيهات ولي الأمر</h3>
                      <p className="text-[8.5pt] font-bold leading-relaxed text-slate-700 whitespace-pre-wrap pr-1">
                        {school.generalMessages || "١. المتابعة المستمرة للمنصة.\n٢. الالتزام بالحضور الصباحي.\n٣. إحضار الكتب المدرسية."}
                      </p>
                   </div>
                   <div className="border-[1.5px] border-slate-900 p-3 bg-white rounded-lg">
                      <h3 className="text-[9.5pt] font-black mb-2 border-b border-slate-900 pb-1 text-center bg-indigo-50 text-indigo-900">القيمة التربوية</h3>
                      <div className="flex flex-col items-center justify-center h-full">
                        ...
                        <p className="text-[10pt] font-black text-center text-indigo-800 leading-snug">
                          {school.weeklyNotes || "البيئة المدرسية الآمنة منطلق الإبداع"}
                        </p>
                      </div>
                   </div>
                </div>
                
                <div className="relative z-10 mt-auto pt-2 flex justify-between items-center px-2 opacity-50 border-t border-slate-100">
                   <p className="text-[7pt] font-black uppercase">خطة المدرسة - {school.name}</p>
                   <div className="text-[7pt] font-black">بوابة مدرستي الرقمية {new Date().getFullYear()}</div>
                </div>
              </div>
            </div>
          </div>
