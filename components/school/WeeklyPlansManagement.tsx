    ...
    <div className="space-y-8 animate-in fade-in font-['Tajawal'] pb-20">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 font-bold mt-1">أرشفة الخطط، إدارة الأسابيع، وتعديل الهوية.</p>
        </div>
      ...
      <div className="flex gap-2 p-1 bg-slate-100 rounded-2xl w-fit">
        {[
          {id:'weeks', label:'الأسابيع النشطة'},
          {id:'archive', label:'الأرشيف التاريخي'},
          ...
          <div className="bg-white p-10 rounded-[3.5rem] border border-slate-100 shadow-sm space-y-8">
             <div className="flex items-center gap-4 mb-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 font-bold">حدد مسمى الأسبوع لتمكين المعلمين من الرصد فيه.</p>
                </div>
             </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-2">
                   ...
                   <input type="text" placeholder="مثال: الأسبوع الثاني" className="w-full p-4 bg-slate-50 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-indigo-100 transition" value={newWeek.name} onChange={e => setNewWeek({...newWeek, name: e.target.value})} />
                </div>
                <div className="space-y-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
             {weeks.map(week => (
               <div key={week.id} className={`bg-white p-8 rounded-[3rem] border-2 transition-all relative overflow-hidden group ${week.isActive ? 'border-indigo-600 shadow-xl shadow-indigo-50' : 'border-slate-100 shadow-sm opacity-80'}`}>
                  ...
                  <p className="text-xs text-slate-400 font-bold mb-6">من {week.startDate} إلى {week.endDate}</p>
                  <button onClick={() => toggleWeekActive(week.id)} disabled={week.isActive} className={`w-full py-3 rounded-xl font-black text-sm flex items-center justify-center gap-2 transition-all ${week.isActive ? 'bg-emerald-500 text-white' : 'bg-slate-900 text-white hover:bg-black'}`}>
                     {week.isActive ? <><CheckCircle size={16} /> متاح للرصد حالياً</> : 'تفعيل للرصد'}
                  </button>
               </div>
             ))}
          </div>
                  ...
                  <div key={archive.id} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-md transition flex justify-between items-center group">
                    <div className="flex items-center gap-5">
                       ...
                       <div>
                          ...
                          <p className="text-xs text-slate-400 font-bold">تاريخ الأرشفة: {new Date(archive.archivedAt).toLocaleDateString('ar-SA')}</p>
                       </div>
                    </div>
                    <button className="bg-slate-900 text-white px-6 py-2.5 rounded-xl font-black text-xs hover:bg-black transition">استعراض البيانات</button>
                  </div>
                ))}
             </div>
           )}
